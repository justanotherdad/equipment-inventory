...
export class Database {
  ...
  initialize() {
    ...
    // Migration: add equipment_number if missing (for existing databases)
    try {
    ...
    // Seed default equipment types if empty
  ...
  addCalibrationRecord(equipmentId: number, sourcePath: string) {
    const fileName = path.basename(sourcePath);
    const ext = path.extname(fileName);
    const uniqueName = `${equipmentId}_${Date.now()}${ext}`;
    const destPath = path.join(this.calRecordsPath, uniqueName);
  ...
  getCalRecordsPath(): string {
    return this.calRecordsPath;
  }
