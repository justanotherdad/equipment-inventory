...
export type AdminWorkspaceMode = 'platform' | 'company';
...
export default function AdminWorkspace({ mode }: { mode: AdminWorkspaceMode }) {
  ...
  const isSuperAdmin = isPlatform;
  const isFullAdmin = isPlatform || profile?.role === 'company_admin';
  const canCreateUser = isFullAdmin || profile?.role === 'equipment_manager';
  const showCompanyPanels = isFullAdmin && (!isPlatform || selectedCompanyId != null);
  ...
  useEffect(() => {
    if (selectedCompanyId && isPlatform) {
      setCompanyForm({});
      setNewUserCompanyId(String(selectedCompanyId));
      api.admin.getCompany(selectedCompanyId).then((c) => setCompanyForm(c)).catch(() => setCompanyForm({}));
    } else if (!isPlatform && profile?.role === 'company_admin' && profile?.company_id) {
      api.admin.getCompany(profile.company_id).then((c) => setCompanyForm(c)).catch(() => setCompanyForm({}));
    } else if (isPlatform && !selectedCompanyId) {
      setCompanyForm({});
    }
  }, [selectedCompanyId, isPlatform, profile?.role, profile?.company_id]);

  useEffect(() => {
    if (showOrderHistory && !isPlatform && profile?.role === 'company_admin') {
      api.payments.getOrders().then(setSubscriptionOrders).catch(() => setSubscriptionOrders([]));
    }
  }, [showOrderHistory, profile?.role, isPlatform]);

  const scopedSites = useMemo(() => {
    if (!isPlatform || !selectedCompanyId) return sites;
    return sites.filter((s) => s.company_id === selectedCompanyId);
  }, [sites, isPlatform, selectedCompanyId]);

  const scopedDepartments = useMemo(() => {
    if (!isPlatform || !selectedCompanyId) return departments;
    const siteIds = new Set(scopedSites.map((s) => s.id));
    return departments.filter((d) => siteIds.has(d.site_id));
  }, [departments, scopedSites, isPlatform, selectedCompanyId]);

  const scopedProfiles = useMemo(() => {
    if (!isPlatform || !selectedCompanyId) return profiles;
    return profiles.filter((p) => p.company_id === selectedCompanyId);
  }, [profiles, isPlatform, selectedCompanyId]);

  const scopedEquipment = useMemo(() => {
    if (!isPlatform || !selectedCompanyId) return equipment;
    const deptIds = new Set(scopedDepartments.map((d) => d.id));
    return equipment.filter((e) => e.department_id != null && deptIds.has(e.department_id));
  }, [equipment, scopedDepartments, isPlatform, selectedCompanyId]);

  useEffect(() => {
    if (scopedSites.length && !scopedSites.some((s) => String(s.id) === newDeptSiteId)) {
      setNewDeptSiteId(String(scopedSites[0].id));
    }
  }, [scopedSites, newDeptSiteId]);
  ...
  const handleSaveCompany = async () => {
    const id = selectedCompanyId ?? (!isPlatform && profile?.role === 'company_admin' ? profile.company_id : null);
    if (!id) return;
    try {
  ...
  const handleAddSite = async (e: React.FormEvent) => {
    ...
    try {
      const companyIdForSite = isPlatform
        ? (selectedCompanyId ?? undefined)
        : undefined;
      if (isPlatform && !companyIdForSite) {
        setError('Select a company before adding a site');
        return;
      }
      await api.admin.createSite(newSiteName.trim(), companyIdForSite);
      setNewSiteName('');
      await load(false);
    } catch (e) {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const companyIdForUser = isPlatform
        ? (newUserCompanyId ? parseInt(newUserCompanyId, 10) : selectedCompanyId ?? undefined)
        : undefined;
  ...
  return (
    <div className="admin-page">
      <div className="page-header">
        <h2>{isPlatform ? 'Platform' : isFullAdmin ? 'Admin Panel' : 'Create User'}</h2>
        <p style={{ color: 'var(--text-muted)' }}>
          {isPlatform
            ? 'Manage companies, subscriptions, and company-level users, sites, and departments.'
            : isFullAdmin
              ? 'Manage users, access levels, sites, and departments.'
              : 'Add a new user within your access scope.'}
        </p>
      </div>
      {error && <p style={{ color: 'var(--danger)', marginBottom: '1rem' }}>{error}</p>}

      {isPlatform && (
        <div className="card admin-subscriptions-col" style={{ marginBottom: '1.5rem' }}>
          <div className="admin-subscriptions-header">
            <div>
              <h3 className="card-title"><CreditCard size={20} /> Companies & Subscriptions</h3>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', margin: 0 }}>
                Enable or disable subscription per company. Select a company to manage its users, sites, and departments.
              </p>
            </div>
            ...
            renderActions={(row) => (
              <div style={{ display: 'flex', gap: '0.25rem' }}>
                <button
                  ...
                  onClick={() => { setSelectedCompanyId(row.id); setEditSubscriptionModal(row); setEditSubscriptionForm(row); }}
                >
                  <Pencil size={14} /> Edit
                </button>
                <button
          ...
          {/* Row 1, Col 1: Company Info (narrower) */}
            <div className="card company-info-compact admin-company-info">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                ...
                {!isPlatform && profile?.role === 'company_admin' && (
                  <button
                    type="button"
                    className="btn btn-danger"
            ...
            <form onSubmit={(e) => {
              ...
              <div className="form-group" style={{ marginBottom: '1rem' }}>
                ...
                >
                  ...
                  {isPlatform && <option value="super_admin">Super Admin</option>}
                </select>
              </div>
